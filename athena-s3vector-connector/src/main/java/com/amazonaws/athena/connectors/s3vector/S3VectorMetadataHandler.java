...
/**
 * This class is part of an S3 Vector connector that enables Athena to query
 * vector data stored in S3. The README for this module (athena-example) will
 * guide you through preparing your development environment, modifying this
 * example Metadatahandler, building, deploying, and then using your new S3
 * vector source in an Athena query.
 *
 ...
 * your S3 vector source contains. Lastly, this class tells Athena how to split
 * up reads against this S3 vector source. This gives you control over the level
 * of performance and parallelism your source can support.
 *
 * <p>For more examples, please see the other connectors in this repository
 * (e.g. athena-cloudwatch, athena-docdb, etc...)
 */
public class S3VectorMetadataHandler extends MetadataHandler {
  private static final Logger logger =
      LoggerFactory.getLogger(S3VectorMetadataHandler.class);
  ...
  /** Constructor for S3VectorMetadataHandler. */
  public S3VectorMetadataHandler(java.util.Map<String, String> configOptions) {
    ...
    this.bucketsCache =
        Suppliers.memoizeWithExpiration(
            this::loadVectorBuckets, CACHE_TTL_SECONDS, TimeUnit.SECONDS);
    this.indexesCache =
   ...
   * @return A ListSchemasResponse which primarily contains a Set of schema
   *     names and a catalog name corresponding the Athena catalog that was
   *     queried.
   */
  @Override
  public ListSchemasResponse doListSchemaNames(
      BlockAllocator allocator, ListSchemasRequest request) {
   ...
   * @return A ListTablesResponse which primarily contains a List of TableName
   ...
   *     queried.
   * @implNote A complete (un-paginated) list of tables should be returned if
   *     the request's pageSize is set to
   *     ListTablesRequest.UNLIMITED_PAGE_SIZE_VALUE.
   */
  @Override
  public ListTablesResponse doListTables(
      BlockAllocator allocator, ListTablesRequest request) {
  ...
  private List<String> loadVectorBuckets() {
    ...
    ListVectorBucketsRequest vectorListRequest =
  ...
  /**
   * Fetches the list of indexes (tables) for a given vector bucket with
   * 30-second caching.
   *
  ...
  private List<TableName> loadIndexesForBucket(String bucketName) {
    ...
    var indexesListRequest =
        ListIndexesRequest.builder().vectorBucketName(bucketName).build();
  ...
  /**
   * Used to get definition (field names, types, descriptions, etc...) of a
   * Table.
   *
   * @param allocator Tool for creating and managing Apache Arrow Blocks.
   * @param request Provides details on who made the request and which Athena
   ...
   *     Schema object describing the table's columns, types, and descriptions.
   *     2. A Set of partition column names (or empty if the table isn't
   *     partitioned). 3. A TableName object confirming the schema and table
   *     name the response is for. 4. A catalog name corresponding the Athena
  ...
  public GetTableResponse doGetTable(BlockAllocator allocator, GetTableRequest request) {
    ...
    tableSchemaBuilder
        ...
        .addListField(
            COL_EMBEDDING_DATA, new ArrowType.FloatingPoint(FloatingPointPrecision.SINGLE))
  ...
  /**
   * Used to get the partitions that must be read from the request table in
   * order to satisfy the requested predicate.
   *
   * @param blockWriter Used to write rows (partitions) into the Apache Arrow
   *     response.
   * @param request Provides details of the catalog, database, and table being
   *     queried as well as any filter predicate.
   * @param queryStatusChecker A QueryStatusChecker that you can use to stop
   *     doing work for a query that has already terminated
   * @note Partitions are partially opaque to Amazon Athena in that it only
   *     understands your partition columns and how to filter out partitions
   *     that do not meet the query's constraints. Any additional columns you
   *     add to the partition data are ignored by Athena but passed on to calls
   *     on GetSplits.
   */
  @Override
   ...
   * partition(s). This is an out-of-the-box (OOTB) implementation that creates
   * a single split for the entire table.
   *
   * @param allocator Tool for creating and managing Apache Arrow Blocks.
   * @param request Provides details of the catalog, database, table, and
   *     partition(s) being queried as well as any filter predicate.
   * @return A GetSplitsResponse which primarily contains: 1. A Set of Split
  ...
  /**
   * Used to describe the types of capabilities supported by a data source. An
   * engine can use this to determine what portions of the query to push down. A
   * connector that returns any optimization will guarantee that the associated
   ...
   *     supported optimizations that the connector is advertising to the
   *     consumer. The connector assumes all responsibility for whatever is
   *     passed here.
   */
  @Override
  public GetDataSourceCapabilitiesResponse doGetDataSourceCapabilities(
    ...
    ImmutableMap.Builder<String, List<OptimizationSubType>> capabilities =
    ...
    return new GetDataSourceCapabilitiesResponse(
        request.getCatalogName(), capabilities.build());
  }
}
