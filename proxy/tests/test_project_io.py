...
class TestProjectExport:
    ...
    @pytest.mark.anyio
    async def test_export_all_projects(self, client):
        p1 = project_store.create(name="Project A")
        p2 = project_store.create(name="Project B")
        projection_store.create(database="db_a", project_id=p1.id)
        projection_store.create(database="db_b", project_id=p2.id)

        resp = await client.get("/api/v0/project/export")
        assert resp.status_code == 200

        data = resp.json()
        assert data["version"] == "1.0"
        assert len(data["projects"]) == 2
        names = {e["project"]["name"] for e in data["projects"]}
