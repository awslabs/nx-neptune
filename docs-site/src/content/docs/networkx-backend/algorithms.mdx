...
- **Neptune Analytics docs:** [Bellman-Ford SSSP Path](https://docs.aws.amazon.com/neptune-analytics/latest/userguide/sssp-bellmanFord-path.html)
- **Source:** [`nx_neptune/algorithms/shortest_paths/bellman_ford.py`](https://github.com/awslabs/nx-neptune/blob/main/nx_neptune/algorithms/shortest_paths/bellman_ford.py)
...
| `weight` | str | Edge weight property name (default: `"weight"`) |
| `edge_weight_type` | str | `"int"`, `"long"`, `"float"`, or `"double"` |
| `edge_labels` | list | Edge label filter |
| `vertex_label` | str | Vertex label filter |
| `traversal_direction` | str | `"inbound"` or `"outbound"` |
...
- **Neptune Analytics docs:** [Bellman-Ford SSSP](https://docs.aws.amazon.com/neptune-analytics/latest/userguide/sssp-bellmanFord.html)
- **Source:** [`nx_neptune/algorithms/shortest_paths/bellman_ford.py`](https://github.com/awslabs/nx-neptune/blob/main/nx_neptune/algorithms/shortest_paths/bellman_ford.py)
...
- **Neptune Analytics docs:** [Bellman-Ford SSSP Parents](https://docs.aws.amazon.com/neptune-analytics/latest/userguide/sssp-bellmanFord-parents.html)
- **Source:** [`nx_neptune/algorithms/shortest_paths/bellman_ford.py`](https://github.com/awslabs/nx-neptune/blob/main/nx_neptune/algorithms/shortest_paths/bellman_ford.py)
...
|-----------|------|-------------|
| `source` | node | Starting node |
| `target` | node | (Unused) Included for NX API compatibility |
| `weight` | str | Edge weight property name (default: `"weight"`) |
| `heuristic` | bool | (Unsupported) Ignored |
| `edge_weight_type` | str | `"int"`, `"long"`, `"float"`, or `"double"` |
| `edge_labels` | list | Edge label filter |
| `vertex_label` | str | Vertex label filter |
| `traversal_direction` | str | `"inbound"` or `"outbound"` |

**Returns:** Tuple of `(pred, dist)` dicts. `pred` maps nodes to `[parent]` lists, `dist` maps nodes to distances.

<Aside type="caution">
  Neptune does not support negative edge weights or `traversalDirection: "both"` for Bellman-Ford algorithms. The `weight` parameter must be a string property name — callable weight functions are not supported.
</Aside>

<Aside type="note">
  Neptune returns a single parent per node. The predecessor list contains at most one element, unlike NetworkX which may return multiple equal-cost predecessors.
</Aside>
